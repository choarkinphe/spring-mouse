...
function getComboModelValue(model) {
  if (typeof model === "string") return model;
  if (model && typeof model.model === "string" && model.model.trim()) return model.model;
  return null;
}
...
export function getComboModelsFromData(modelStr, combosData) {
  ...
  if (combo && combo.isActive !== false && combo.models && combo.models.length > 0) {
    return combo.models.map(getComboModelValue).filter(Boolean);
  }
  return null;
}
