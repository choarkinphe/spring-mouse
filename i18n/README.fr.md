<div align="center">
  <img src="../images/spring-mouse.png?1" alt="Tableau de bord Spring Mouse" width="800"/>
  
  # Spring Mouse - Routeur IA GRATUIT et économiseur de tokens
...
[🚀 Démarrage rapide](#-démarrage-rapide) • [💡 Fonctionnalités](#-fonctionnalités-clés) • [📖 Configuration](#-guide-dinstallation) • [🌐 Site web](https://spring-mouse.com)
...
🎉 Le tableau de bord s'ouvre sur `http://localhost:8008`
...
Le paquet de ce dépôt est privé (`spring-mouse-app`), donc l'exécution depuis le code source/Docker est le chemin de développement local attendu.
...
- Tableau de bord : `http://localhost:8008/dashboard`
- API compatible OpenAI : `http://localhost:8008/v1`
...
> 🎬 **Vous avez réalisé une vidéo sur Spring Mouse ?** Envoyez une [Pull Request](https://github.com/decolua/spring-mouse/pulls) en ajoutant votre vidéo à cette section — nous la fusionnerons !

---

## 🛠️ Outils CLI pris en charge

Spring Mouse fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. Spring Mouse appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → Spring Mouse → Headroom /v1/compress → Spring Mouse → fournisseur
```
...
Si Headroom est indisponible ou renvoie une erreur, Spring Mouse bascule en mode dégradé et envoie la requête originale.
...
- Votre outil CLI envoie le format OpenAI → Spring Mouse le traduit → Le fournisseur reçoit le format natif
...
> Spring Mouse lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de Spring Mouse

**La réalité de la facturation de Spring Mouse :**

✅ **Le logiciel Spring Mouse = GRATUIT pour toujours** (open source, ne facture jamais)  
...
❌ **Spring Mouse n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, Spring Mouse se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **Spring Mouse** : Ne facture jamais rien, jamais
...
<details>
<summary><b>💳 Spring Mouse me facturera-t-il ?</b></summary>

**Non.** Spring Mouse est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, Spring Mouse route simplement vos requêtes
- ❌ **Spring Mouse lui-même** → **Ne facture jamais rien, jamais**

Spring Mouse est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
Spring Mouse se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et Spring Mouse les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - Spring Mouse vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de Spring Mouse évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et Spring Mouse les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. Spring Mouse suit le quota par modèle !
...
Settings → Models → Advanced :
  ...
  OpenAI API Key : [depuis le tableau de bord spring-mouse]
...
```json
{
  ...
  "anthropic_api_key": "votre-clé-api-spring-mouse"
}
```
...
export OPENAI_API_KEY="votre-clé-api-spring-mouse"
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un Spring Mouse local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
docker pull decolua/spring-mouse:latest   # mise à jour vers la dernière version
```

**Persistance des données :** `$HOME/.spring-mouse/db/data.sqlite` sur l'hôte ↔ `/app/data/db/data.sqlite` dans le conteneur.

### Variables d'environnement

| Variable                                             | Par défaut                                  | Description                                                                                  |
| ---------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Généré automatiquement (`~/.spring-mouse/jwt-secret`) | Secret de signature JWT pour le cookie d'authentification du tableau de bord (remplacez-le pour partager entre instances) |
| `INITIAL_PASSWORD`                                   | `123456`                                    | Mot de passe de première connexion quand aucun hash n'est enregistré                          |
| `DATA_DIR`                                           | `~/.spring-mouse`                                | Emplacement principal des données de l'app (SQLite dans `$DATA_DIR/db/data.sqlite`)          |
| `PORT`                                               | défaut du framework                         | Port du service (`8008` dans les exemples)                                                   |
...
| `BASE_URL`                                           | `http://localhost:8008`                    | URL de base interne côté serveur utilisée par les tâches de synchronisation cloud             |
| `CLOUD_URL`                                          | `https://spring-mouse.com`                       | URL de base de l'endpoint de synchronisation cloud côté serveur                               |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                     | URL de base publique/rétrocompatible (préférez `BASE_URL` pour le runtime serveur)            |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://spring-mouse.com`                       | URL cloud publique/rétrocompatible (préférez `CLOUD_URL` pour le runtime serveur)             |
...
| `ENABLE_REQUEST_LOG_FILE_DUMPS`                                | `false`                                     | Active les journaux de requêtes/réponses dans `logs/`                                         |
...
- Journaux optionnels de requêtes/translator : `<repo>/logs/...` quand `ENABLE_REQUEST_LOG_FILE_DUMPS=true`
- `${DATA_DIR}` et `~/.spring-mouse` résolvent tous deux le même emplacement dans un conteneur Docker — le lien symbolique `/root/.spring-mouse -> /app/data` est créé au moment de la compilation.
...
- Renouvelé automatiquement par Spring Mouse
...
- Définissez `PORT=8008` et `NEXT_PUBLIC_BASE_URL=http://localhost:8008`
...
- Définissez `ENABLE_REQUEST_LOG_FILE_DUMPS=true`
...
- **Site web** : [spring-mouse.com](https://spring-mouse.com)
- **GitHub** : [github.com/decolua/spring-mouse](https://github.com/decolua/spring-mouse)
- **Problèmes** : [github.com/decolua/spring-mouse/issues](https://github.com/decolua/spring-mouse/issues)

---

## 👥 Contributeurs

Merci à tous les contributeurs qui ont contribué à améliorer Spring Mouse !
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. Spring Mouse porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. Spring Mouse adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. Spring Mouse injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de Spring Mouse n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
