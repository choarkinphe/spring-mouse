/**
 * `spring-mouse xai video` — generate a Grok Imagine video through the local
 * Spring Mouse gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: spring-mouse xai video --prompt "..." [options]

Generate a Grok Imagine video via your local Spring Mouse gateway
...
Options:
  ...
  --api-key <key>         Spring Mouse API key (or env SPRING_MOUSE_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.SPRING_MOUSE_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-spring-mouse-connection-id"] || null;
