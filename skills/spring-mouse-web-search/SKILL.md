---
name: spring-mouse--search
description: Web search via Spring Mouse /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# Spring Mouse — Web Search
...
```bash
curl $SPRING_MOUSE_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$SPRING_MOUSE_URL/v1/models/info?id=tavily/search"
```
...
`POST $SPRING_MOUSE_URL/v1/search`
...
```bash
curl -X POST $SPRING_MOUSE_URL/v1/search \
  -H "Authorization: Bearer $SPRING_MOUSE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"Spring Mouse open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.SPRING_MOUSE_URL}/v1/search`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.SPRING_MOUSE_KEY}`, "Content-Type": "application/json" },
...
{
  "provider": "tavily",
  "query": "Spring Mouse open source",
  "results": [
    {
