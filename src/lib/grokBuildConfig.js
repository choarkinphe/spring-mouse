...
const previousDefaultRegExp = /^# spring-mouse-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# spring-mouse-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Spring Mouse gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# spring-mouse-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# spring-mouse-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
