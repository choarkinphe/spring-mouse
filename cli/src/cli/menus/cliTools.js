...
async function buildCodexHeader() {
  ...
  const { installed, hasSpringMouse, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasSpringMouse, settings } = result.data;
  ...
  // Extract Spring Mouse custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:SpringMouse-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Spring Mouse provider config
  const provider = settings?.models?.providers?.["spring-mouse"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("spring-mouse/") ? primary.replace("spring-mouse/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasSpringMouse, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Spring Mouse\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
