...
beforeEach(() => {
  tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "spring-mouse-mig-"));
...
"apiKeys", "combos", "kv", "usageHistory", "usageDaily", "networkTraffic", "requestDetails",
  ...
  it("version 8 copies legacy resets to both windows and disables off keys", async () => {
    const { getAdapter } = await import("@/lib/db/driver.js");
    const db = await getAdapter();
    const legacyResetAt = new Date(Date.now() - 60 * 60 * 1000).toISOString();
    db.run(
      `INSERT INTO apiKeys(id, key, name, machineId, isActive, quotaMode, quotaResetAt, createdAt) VALUES(?, ?, ?, ?, 1, ?, ?, ?)`,
      ["legacy-reset", "sk-legacy-reset", "legacy", "machine", "off", legacyResetAt, new Date().toISOString()],
    );
