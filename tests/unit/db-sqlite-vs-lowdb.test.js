...
beforeAll(async () => {
  tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "spring-mouse-db-compare-"));
...
describe("DB SQLite layer — public API parity", () => {
  ...
  it("combos: CRUD", async () => {
    const c = await sqliteDb.createCombo({
      name: "combo1", models: ["m1", "m2"], kind: "fallback", groupName: "生产", sortOrder: 20,
    });
    expect(c.id).toBeDefined();
    expect(c.models).toEqual(["m1", "m2"]);
    expect(c.isActive).toBe(true);
    expect(c.groupName).toBe("生产");
    expect(c.sortOrder).toBe(20);
    const byName = await sqliteDb.getComboByName("combo1");
    expect(byName.id).toBe(c.id);
    await sqliteDb.updateCombo(c.id, { models: ["m3"], isActive: false, groupName: "测试", sortOrder: -10 });
    const updated = await sqliteDb.getComboById(c.id);
    expect(updated.models).toEqual(["m3"]);
    expect(updated.isActive).toBe(false);
    expect(updated.groupName).toBe("测试");
    expect(updated.sortOrder).toBe(-10);
    const second = await sqliteDb.createCombo({ name: "combo2", models: ["m4"], groupName: "测试", sortOrder: 0 });
    const ordered = (await sqliteDb.getCombos()).filter((combo) => combo.groupName === "测试");
    expect(ordered.map((combo) => combo.id)).toEqual([c.id, second.id]);
    expect(await sqliteDb.deleteCombo(c.id)).toBe(true);
    expect(await sqliteDb.deleteCombo(second.id)).toBe(true);
  });
  ...
  it("usage: saveRequestUsage + getUsageHistory + getUsageStats", async () => {
    const recentUser = await sqliteDb.createApiKey("近期请求使用人", "recent-user");
    await sqliteDb.saveRequestUsage({
    ...
    expect(stats.recentRequests.some((request) => request.userName === "近期请求使用人")).toBe(true);
  });
