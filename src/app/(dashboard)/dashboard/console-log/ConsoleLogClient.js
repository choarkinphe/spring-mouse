...
function statusLabel(status) {
  ...
  if (status.startsWith("upstream:")) return `模型上游错误（${status.slice(9)}）`;
  if (status.startsWith("blocked:")) return `路由策略拦截（${status.slice(8)}）`;
  if (status.startsWith("error:")) return `错误（${status.slice(6)}）`;
  return status;
}
...
export default function ConsoleLogClient() {
  ...
  const loadInternalStats = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/usage/internal-stats", { cache: "no-store" });
      const payload = await response.json();
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden">
        ...
        <div className="grid h-full min-h-0 lg:grid-cols-[19rem_minmax(0,1fr)]">
          ...
          <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-black/[0.02] dark:bg-black/20">
            {activeView.type === "console" ? (
              ...
              <>
                <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3"><div><h3 className="font-semibold">请求统计</h3><p className="text-xs text-text-muted">最近 100 条真实请求；不足 100 条时按实际请求数统计</p></div><button type="button" onClick={loadInternalStats} className="rounded p-1 text-text-muted hover:bg-surface-2"><span className={`material-symbols-outlined text-[18px] ${loadingStats ? "animate-spin" : ""}`}>refresh</span></button></div>
                {loadingStats && !internalStats ? <div className="flex flex-1 items-center justify-center text-sm text-text-muted">正在读取统计…</div> : internalStats ? <div className="custom-scrollbar min-h-0 flex-1 overflow-auto p-4"><div className="grid grid-cols-2 gap-3 md:grid-cols-5"><div className="rounded-lg border border-border p-3"><p className="text-xs text-text-muted">总请求</p><p className="mt-1 text-2xl font-semibold">{internalStats.total}</p></div><div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3"><p className="text-xs text-text-muted">成功</p><p className="mt-1 text-2xl font-semibold text-emerald-400">{internalStats.success}</p></div><div className="rounded-lg border border-rose-500/20 bg-rose-500/5 p-3"><p className="text-xs text-text-muted">中转错误（Spring Mouse）</p><p className="mt-1 text-2xl font-semibold text-rose-400">{internalStats.internal}</p></div><div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3"><p className="text-xs text-text-muted">模型上游错误</p><p className="mt-1 text-2xl font-semibold text-amber-400">{internalStats.upstream}</p></div><div className="rounded-lg border border-sky-500/20 bg-sky-500/5 p-3"><p className="text-xs text-text-muted">路由策略拦截</p><p className="mt-1 text-2xl font-semibold text-sky-400">{internalStats.blocked ?? 0}</p></div></div><h4 className="mt-6 mb-2 text-sm font-semibold">状态明细</h4><div className="space-y-2">{internalStats.byStatus.map((item) => <div key={item.status} className="flex items-center justify-between rounded border border-border px-3 py-2 text-sm"><span className="font-mono text-text-muted">{statusLabel(item.status)}</span><span className="font-semibold tabular-nums">{item.count}</span></div>)}</div><div className="mt-6 mb-2 flex items-center justify-between"><h4 className="text-sm font-semibold">最新 10 条请求</h4><button type="button" onClick={() => setAllRequestsOpen(true)} className="text-xs text-brand-400 hover:text-brand-300">查看更多</button></div><div className="overflow-x-auto rounded-lg border border-border"><table className="w-full text-left text-xs"><thead className="bg-surface-2 text-text-muted"><tr><th className="px-3 py-2">时间</th><th className="px-3 py-2">模型</th><th className="px-3 py-2">提供商</th><th className="px-3 py-2">调用人</th><th className="px-3 py-2">耗时</th><th className="px-3 py-2">状态</th><th className="px-3 py-2">Token</th><th className="px-3 py-2">明细</th></tr></thead><tbody>{recentRequests.slice(0, 10).map((item) => <tr key={item.id} className="cursor-pointer border-t border-border hover:bg-surface-2" onClick={() => openRequestDetail(item)}><td className="whitespace-nowrap px-3 py-2 text-text-muted">{item.timestamp ? new Date(item.timestamp).toLocaleString("zh-CN", { hour12: false }) : "-"}</td><td className="max-w-[14rem] truncate px-3 py-2 font-mono" title={item.model}>{item.model || "-"}</td><td className="px-3 py-2">{item.provider || "-"}</td><td className="max-w-[10rem] truncate px-3 py-2" title={item.caller}>{item.caller || "-"}</td><td className="whitespace-nowrap px-3 py-2 tabular-nums">{item.durationMs == null ? "-" : `${item.durationMs} ms`}</td><td className="px-3 py-2 font-mono" title={item.status}>{statusLabel(item.status)}</td><td className="px-3 py-2 tabular-nums">{(Number(item.promptTokens) || 0) + (Number(item.completionTokens) || 0)}</td><td className="px-3 py-2 text-brand-400">查看</td></tr>)}</tbody></table>{recentRequests.length === 0 && <p className="px-3 py-4 text-center text-xs text-text-muted">暂无请求记录。</p>}</div></div> : <div className="flex flex-1 items-center justify-center text-sm text-text-muted">暂无统计。</div>}
              </>
            ) : (
              <>
      ...
      {allRequestsOpen && <div className="fixed inset-0 z-[80] bg-black/40" onClick={() => setAllRequestsOpen(false)}><div className="absolute inset-y-0 right-0 flex w-[min(1080px,98vw)] flex-col border-l border-border bg-surface shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3"><div><h3 className="font-semibold">全部请求记录</h3><p className="text-xs text-text-muted">最近 100 条真实请求</p></div><button type="button" onClick={() => setAllRequestsOpen(false)} className="rounded p-1 text-text-muted hover:bg-surface-2" aria-label="关闭全部请求记录"><span className="material-symbols-outlined">close</span></button></div><div className="custom-scrollbar min-h-0 flex-1 overflow-auto p-4"><div className="overflow-x-auto rounded-lg border border-border"><table className="w-full text-left text-xs"><thead className="bg-surface-2 text-text-muted"><tr><th className="px-3 py-2">时间</th><th className="px-3 py-2">模型</th><th className="px-3 py-2">提供商</th><th className="px-3 py-2">调用人</th><th className="px-3 py-2">耗时</th><th className="px-3 py-2">状态</th><th className="px-3 py-2">Token</th><th className="px-3 py-2">明细</th></tr></thead><tbody>{recentRequests.map((item) => <tr key={item.id} className="cursor-pointer border-t border-border hover:bg-surface-2" onClick={() => openRequestDetail(item)}><td className="whitespace-nowrap px-3 py-2 text-text-muted">{item.timestamp ? new Date(item.timestamp).toLocaleString("zh-CN", { hour12: false }) : "-"}</td><td className="max-w-[14rem] truncate px-3 py-2 font-mono" title={item.model}>{item.model || "-"}</td><td className="px-3 py-2">{item.provider || "-"}</td><td className="max-w-[10rem] truncate px-3 py-2" title={item.caller}>{item.caller || "-"}</td><td className="whitespace-nowrap px-3 py-2 tabular-nums">{item.durationMs == null ? "-" : `${item.durationMs} ms`}</td><td className="px-3 py-2 font-mono" title={item.status}>{statusLabel(item.status)}</td><td className="px-3 py-2 tabular-nums">{(Number(item.promptTokens) || 0) + (Number(item.completionTokens) || 0)}</td><td className="px-3 py-2 text-brand-400">查看</td></tr>)}</tbody></table>{recentRequests.length === 0 && <p className="px-3 py-4 text-center text-xs text-text-muted">暂无请求记录。</p>}</div></div></div></div>}

      {selectedRequest && <div className="fixed inset-y-0 right-0 z-[90] flex w-[min(760px,96vw)] flex-col border-l border-border bg-surface shadow-2xl"><div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3"><div><h3 className="font-semibold">请求明细</h3><p className="text-xs text-text-muted">{selectedRequest.timestamp ? new Date(selectedRequest.timestamp).toLocaleString("zh-CN", { hour12: false }) : ""} · {selectedRequest.model || "未知模型"}</p></div><button type="button" onClick={() => setSelectedRequest(null)} className="rounded p-1 text-text-muted hover:bg-surface-2" aria-label="关闭请求明细"><span className="material-symbols-outlined">close</span></button></div>{loadingRequest ? <div className="flex flex-1 items-center justify-center text-sm text-text-muted">正在读取…</div> : <div className="custom-scrollbar min-h-0 flex-1 space-y-4 overflow-auto p-4">{selectedRequest._notice && <div className="rounded border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">{selectedRequest._notice}</div>}<div className="grid grid-cols-2 gap-2 text-xs"><div className="rounded border border-border p-2"><span className="text-text-muted">状态</span><p className="mt-1 font-mono">{selectedRequest.status || "-"}</p></div><div className="rounded border border-border p-2"><span className="text-text-muted">提供商 / 模型</span><p className="mt-1 break-all font-mono">{selectedRequest.provider || "-"} / {selectedRequest.model || "-"}</p></div><div className="rounded border border-border p-2"><span className="text-text-muted">TTFT</span><p className="mt-1">{selectedRequest.latency?.ttft ?? "-"} ms</p></div><div className="rounded border border-border p-2"><span className="text-text-muted">总耗时</span><p className="mt-1">{selectedRequest.latency?.total ?? "-"} ms</p></div><div className="rounded border border-border p-2"><span className="text-text-muted">执行节点</span><p className="mt-1 break-all font-mono">{selectedRequest.mouse?.name || (selectedRequest.mouse?.id ? String(selectedRequest.mouse.id).slice(0, 8) : "-")}</p></div></div>{["request", "providerRequest", "providerResponse", "response"].map((key) => <section key={key}><h4 className="mb-1 text-sm font-semibold">{key}</h4><pre className="max-h-72 overflow-auto whitespace-pre-wrap break-words rounded border border-border bg-black/30 p-3 font-mono text-[11px] leading-5">{JSON.stringify(selectedRequest[key] ?? null, null, 2)}</pre></section>)}</div>}</div>}
