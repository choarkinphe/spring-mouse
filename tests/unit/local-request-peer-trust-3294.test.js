...
// path, so x-sm-real-ip arrives straight from the client and a remote caller can claim to
...
describe("peer header trust", () => {
  ...
  it("accepts the legacy Host fallback only in development", async () => {
    ...
    const response = await proxy(request("/api/v1/models", { host: "localhost:8007" }));
...
describe("login limiter client IP", () => {
  ...
  it("buckets spoofed peer IPs together so lockout cannot be rotated away", () => {
    const first = getClientIp(request("/api/auth/login", { "x-sm-real-ip": "1.1.1.1" }));
    const second = getClientIp(request("/api/auth/login", { "x-sm-real-ip": "2.2.2.2" }));
  ...
  it("still honours TRUST_PROXY for operators fronting Spring Mouse with a reverse proxy", () => {
