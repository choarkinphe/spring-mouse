...
export default function ComboDetailPage() {
  ...
  const handleToggleActive = async (isActive) => {
    const ok = await saveCombo({ isActive });
    if (ok) setCombo((current) => current ? { ...current, isActive } : current);
  };
  ...
  const curlExample = examplePath
    ? `curl -X POST http://localhost:8008${examplePath} \\\n  -H "Content-Type: application/json" \\\n  -H "Authorization: Bearer ${apiKey || "YOUR_KEY"}" \\\n  -d '${JSON.stringify(exampleBody)}'`
    : "";
      ...
      <Card>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium">启用组合</p>
              <p className="text-xs text-text-muted">禁用后，该组合不会出现在模型列表中，也不会参与路由。</p>
            </div>
