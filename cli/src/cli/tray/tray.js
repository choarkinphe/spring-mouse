...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `Spring Mouse (Port ${port})`, tooltip: "Server is running", enabled: false },
...
function initWindowsTray(options) {
  ...
  try {
    ...
    trayInstance = initWinTray({
      iconPath,
      tooltip: `Spring Mouse - Port ${port}`,
      items,
      onClick: (index) => {
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[spring-mouse] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[spring-mouse] tray init error: ${err.message}\n`);
    return null;
  }
}
