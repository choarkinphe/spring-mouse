...
function checkStoreLock(store, ip) {
  const e = getEntryFor(store, ip);
...
export function checkLock(ip) {
  return checkStoreLock(attempts, ip);
}

function recordStoreFail(store, ip) {
  const e = getEntryFor(store, ip) || { fails: 0, lockUntil: 0, lockLevel: 0, lastFailAt: 0 };
  e.fails += 1;
  e.lastFailAt = now();
...
export function recordFail(ip) {
  return recordStoreFail(attempts, ip);
}

export function checkMfaLock(ip) {
  return checkStoreLock(mfaAttempts, ip);
}

export function recordMfaFail(ip) {
  return recordStoreFail(mfaAttempts, ip);
}

export function recordMfaSuccess(ip) {
  mfaAttempts.delete(ip);
}
...
export function getClientIp(request) {
  ...
  if (hasTrustedPeerHeaders(request)) {
    const realIp = request.headers.get("x-sm-real-ip");
