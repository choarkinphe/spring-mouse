...
describe("calculateCostFromTokens (canonical inclusive convention)", () => {
  ...
  it("charges reasoning once when it is already included in completion_tokens", () => {
    const reasoningPricing = { input: 1, output: 10, reasoning: 15 };
    const cost = calculateCostFromTokens(
      { prompt_tokens: 100, completion_tokens: 50, reasoning_tokens: 10 },
      reasoningPricing,
    );
    // 50 output includes 10 reasoning tokens: 40 × output + 10 × reasoning.
    expect(cost).toBeCloseTo((100 * 1 + 40 * 10 + 10 * 15) / 1_000_000, 12);
  });
});
...
describe("OpenAI-compatible usage tracking", () => {
  it("reads usage attached to a completed choice", () => {
  ...
  it("requests authoritative usage for official OpenAI chat-completion streams", () => {
    const executor = new DefaultExecutor("openai");
    const body = { messages: [{ role: "user", content: "hello" }], stream_options: { include_usage: false } };
...
describe("Gemini usage tracking", () => {
  ...
  it("counts reasoning tokens in streaming completion usage", () => {
    const usage = extractUsage({ usageMetadata });
    expect(usage).toMatchObject({
      prompt_tokens: 100,
      completion_tokens: 50,
      total_tokens: 150,
    ...
    expect(canonicalizeUsage(usage).total_tokens).toBe(150);
  });

  it("derives omitted candidate tokens from the provider total", () => {
    const usage = extractUsage({ usageMetadata: { ...usageMetadata, candidatesTokenCount: 0 } });
    expect(usage.completion_tokens).toBe(50);
  });

  it("uses the same billable completion total for non-streaming responses", () => {
    const usage = extractUsageFromResponse({ response: { usageMetadata } });
    expect(usage).toMatchObject({ prompt_tokens: 100, completion_tokens: 50, reasoning_tokens: 10 });
    expect(canonicalizeUsage(usage).total_tokens).toBe(150);
  });
});
