...
Spring Mouse v0.1.0 — first internal release. Baseline: 9Router v0.5.55
(2026-08-14) upstream snapshot; the version line restarts at 0.1.0 and is
bumped per internal release from here on.

## Fork identity
- **Project**: renamed to Spring Mouse (repo `service/spring-mouse`, image
  `docker.wiguo.cn/spring-mouse:latest`); sidebar brand simplified to a
  single centered "Spring Mouse" line; favicon / PWA icons now the `>_`
  terminal mark (deep navy + sky blue) matching the sidebar logo
- **Ports**: runtime 8008 (was 20128), dev 8007 (was 20127) — across
  package.json scripts, Dockerfile, compose, Jenkinsfile, src defaults,
  tests, docs
- **Data dir**: `~/.spring-mouse` (was `~/.9router`) with one-time
  automatic migration on first start (rename, cross-device copy fallback);
  `DATA_DIR` env still takes precedence, Docker layout unchanged
- **Updates**: `/api/version` no longer compares against the npm `spring-mouse`
  upstream (internal releases tracked by git + Jenkins image revision
  labels); hasUpdate is always false

## Fixes
- **Usage stats**: byAccount daily buckets now key on
  `connectionId|model|provider` so multi-model accounts keep per-model
  token splits in 7d/30d/60d/all views; 24h/today local-no-key byApiKey
  buckets split per model (were merged into one)
- **Headroom dashboard**: a reachable sidecar proxy now counts as
  "installed" — the compression panel ([code]/[ml]) renders in docker
  deployments instead of being hidden behind a local-CLI check

## CI/CD
- Jenkins pipeline: checkout -> docker build (Aliyun apk mirror, npmmirror
  registry, legacy-builder compatible Dockerfile) -> label verification ->
  push -> deploy with compose bootstrap, pull retry, /api/health +
  /api/version checks, triple image/container/build-info consistency gates
- Dockerfile: CI-friendly mirrors, APP_BUILD_VERSION revision label,
  /app/build-info.json provenance
...
- **RTK**: `X-Spring-Mouse-Token-Saver` header to bypass token savers per request
