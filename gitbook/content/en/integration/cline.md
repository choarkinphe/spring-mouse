...
Integrate Spring Mouse with Cline VSCode extension to route your AI requests through Spring Mouse's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- Spring Mouse running locally or cloud endpoint configured
...
Set the base URL to your Spring Mouse endpoint:

**For Local Spring Mouse:**
```
http://localhost:8008/v1
```

**For Cloud Spring Mouse:**
```
https://spring-mouse.com
```

**Steps:**
1. In the **Base URL** field, enter your Spring Mouse endpoint
...
3. The key should start with `sk-spring-mouse-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your Spring Mouse configuration
...
4. Cline will use Spring Mouse to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using Spring Mouse
...
3. Get AI-powered explanations through Spring Mouse

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use Spring Mouse to understand context and make changes
...
1. Verify Spring Mouse is running: `curl http://localhost:8008/health`
2. Check that the base URL is correct and includes `/v1`
3. Ensure no firewall is blocking port 8008
...
2. Verify your Spring Mouse instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check Spring Mouse logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use Spring Mouse cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://spring-mouse.com`
2. Make sure you have configured your API key in the Spring Mouse cloud dashboard
...
## Integration with Spring Mouse Features

### Model Routing

Spring Mouse automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, Spring Mouse automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through Spring Mouse dashboard:
- Total requests
- Token usage
- Cost per model
