<div align="center">
  ...
  **Hubungkan semua tool AI coding (Claude Code, Cursor, Antigravity, Copilot, Codex, Gemini, OpenCode, Cline, OpenClaw...) ke 40+ provider AI dan 100+ model.**
  ...
  [🚀 Mulai Cepat](#-mulai-cepat) • [💡 Fitur](#-fitur-utama) • [📖 Setup](#-panduan-setup) • [🌐 Website](https://spring-mouse.com)
...
**Spring Mouse menyelesaikan itu semua:**
...
🎉 Dashboard terbuka di `http://localhost:8008`
...
Paket repo ini bersifat privat (`spring-mouse-app`), jadi menjalankan dari source/Docker adalah jalur yang diharapkan untuk pengembangan lokal.
...
- API kompatibel OpenAI: `http://localhost:8008/v1`
...
### 📺 Panduan Setup Lengkap - Spring Mouse + Claude Code Gratis
...
- ✅ Install dan setup Spring Mouse
...
Spring Mouse bekerja mulus dengan semua tool AI coding utama:
...
- Tool CLI mengirim dalam format OpenAI → Spring Mouse mengonversi → provider menerima dalam format nativenya
...
> Spring Mouse sendiri **tidak menagih apa pun**. Kamu hanya membayar langsung ke provider jika memakai layanan berbayar.
>
...
>
> Anggap saja ini "pelacak penghematan" yang menunjukkan berapa banyak yang kamu hemat lewat model gratis dan routing Spring Mouse!
...
### 📊 Tentang Biaya dan Penagihan Spring Mouse

**Fakta soal penagihan Spring Mouse:**

✅ **Software Spring Mouse = gratis selamanya** (open source, tanpa tagihan)
...
❌ **Spring Mouse tidak mengirim invoice** atau menagih kartumu
...
- **Provider murah** (GLM, MiniMax): bayar langsung, Spring Mouse hanya melakukan routing
- **Provider gratis** (iFlow, Kiro, Qwen): benar-benar gratis selamanya, tanpa biaya tersembunyi
- **Spring Mouse**: tidak menagih apa pun
...
Dashboard melacak pemakaian token dan menampilkan **estimasi biaya** seandainya kamu memakai API berbayar secara langsung. Ini **bukan tagihan nyata**, melainkan acuan untuk melihat berapa banyak yang kamu hemat dengan memakai model gratis atau langganan yang sudah ada lewat Spring Mouse.
...
<details>
<summary><b>💳 Apakah Spring Mouse menagih saya?</b></summary>

**Tidak.** Spring Mouse adalah software open source gratis yang berjalan di komputermu sendiri. Tidak ada penagihan sama sekali.

**Kamu membayar ke:**
- ✅ **Provider langganan** (Claude Code $20/bulan, Codex $20-200/bulan) → bayar langsung di website masing-masing
- ✅ **Provider murah** (GLM, MiniMax) → bayar langsung, Spring Mouse hanya me-routing request
- ❌ **Spring Mouse sendiri** → **tidak menagih apa pun**

Spring Mouse adalah proxy/router lokal. Ia tidak menyimpan informasi kartu kredit, tidak bisa mengirim invoice, dan tidak punya sistem penagihan. Sepenuhnya software gratis.
...
Spring Mouse hanya me-routing request — tidak ada "jebakan" atau tagihan di kemudian hari. Layanannya memang gratis, dan Spring Mouse membuatnya lebih mudah dipakai dengan dukungan fallback.

**Catatan:** beberapa provider langganan (Antigravity, GitHub Copilot) punya masa preview gratis dan bisa jadi berbayar nanti, tetapi hal itu diumumkan secara jelas oleh provider tersebut, bukan oleh Spring Mouse.
...
3. **Gunakan provider langganan paling akhir:**
   ...
   - Spring Mouse memaksimalkan nilainya lewat pelacakan kuota
...
Smart fallback Spring Mouse mencegah tagihan tak terduga:
...
**Kamu yang pegang kendali:** atur batas pengeluaran per provider di dashboard, dan Spring Mouse akan mematuhinya.
...
**Tips pro:** pakai Opus untuk tugas kompleks, Sonnet kalau mengutamakan kecepatan. Spring Mouse melacak kuota per model!
...
docker run -d \
  ...
  ghcr.io/decolua/spring-mouse:latest
```
...
```bash
npm install -g spring-mouse
PORT=8008 HOSTNAME=0.0.0.0 BASE_URL=https://your-domain.com spring-mouse
```
...
|---------|----------------------|--------|
| Tool CLI tidak bisa konek | Endpoint salah | Pastikan `http://localhost:8008/v1` |
...
MIT License — lihat [LICENSE](https://github.com/decolua/spring-mouse/blob/main/LICENSE) untuk detailnya.
