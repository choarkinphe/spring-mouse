...
Preguntas comunes sobre Spring Mouse.

---

## ¿Qué es Spring Mouse?

**Spring Mouse es un router de modelos de IA que maximiza el valor de tu suscripción y minimiza los costos.**
...
**Spring Mouse usa una estrategia de precios de 3 niveles:**
...
## ¿Spring Mouse es gratis?

**Sí, Spring Mouse en sí es 100% gratis y open source.**
...
**¡Sí! Esta es la característica principal de Spring Mouse.**
...
**Spring Mouse rastrea la cuota en tiempo real para todos los proveedores:**
...
## ¿Spring Mouse funciona con Cursor?

**Sí, pero Cursor requiere un endpoint en la nube.**

**Problema:** Cursor IDE no soporta endpoints en localhost.

**Solución:** Usa el despliegue en la nube de Spring Mouse:

```
Cursor Settings → Models → Advanced:
  OpenAI API Base URL: https://spring-mouse.com/v1
...
## ¿Puedo auto-hospedar Spring Mouse?

**¡Sí! Spring Mouse soporta múltiples opciones de despliegue:**
...
- `DATA_DIR` - Ruta de almacenamiento de la base de datos (por defecto: `~/.spring-mouse`)
...
**Sí, Spring Mouse prioriza la seguridad y privacidad:**

**Almacenamiento local:**
- Todos los datos se almacenan localmente en `~/.spring-mouse` (o `DATA_DIR` personalizado)
- No se envían datos a los servidores de Spring Mouse
...
### Docker
```bash
docker pull spring-mouse:latest
docker stop spring-mouse
docker rm spring-mouse
docker run -d \
  -p 3000:3000 \
  -v spring-mouse-data:/app/data \
  spring-mouse:latest
...
**Verificar versión:**
```bash
spring-mouse --version
```

**Cambios disruptivos:**
- Revisa [CHANGELOG.md](https://github.com/decolua/spring-mouse/blob/main/CHANGELOG.md)
- Respalda `~/.spring-mouse` antes de actualizaciones mayores
...
1. **Reportar bugs:**
   - [GitHub Issues](https://github.com/decolua/spring-mouse/issues)
   - Incluye logs de error, pasos para reproducir

2. **Solicitar características:**
   - [GitHub Discussions](https://github.com/decolua/spring-mouse/discussions)
   - Describe el caso de uso y los beneficios

3. **Enviar código:**
   ```bash
   # Fork del repo
   git clone https://github.com/YOUR_USERNAME/spring-mouse.git
   cd spring-mouse
...
Consulta [CONTRIBUTING.md](https://github.com/decolua/spring-mouse/blob/main/CONTRIBUTING.md) para detalles.

---

## ¿Necesitas más ayuda?

- **Documentación:** [spring-mouse.com/docs](https://spring-mouse.com/docs)
