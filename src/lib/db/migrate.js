...
function importLegacyMain(adapter, data) {
  ...
  importWithAssertion(adapter, "apiKeys", data.apiKeys || [], (k) => {
    adapter.run(
      `INSERT OR REPLACE INTO apiKeys(id, key, name, machineId, isActive, quotaMode, quotaResetAt, fiveHourQuotaResetAt, weeklyQuotaResetAt, createdAt, lastUsedAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, normalizeApiKeyQuotaMode(k.quotaMode), k.quotaResetAt || null, k.fiveHourQuotaResetAt || null, k.weeklyQuotaResetAt || null, k.createdAt || new Date().toISOString(), k.lastUsedAt || null]
    );
...
function legacyUsageApiKeyId(adapter, apiKey) {
  if (!apiKey) return "local-no-key";
  const row = adapter.get(`SELECT id FROM apiKeys WHERE key = ?`, [apiKey]);
  if (row?.id) return row.id;
  const secret = adapter.get(`SELECT value FROM _meta WHERE key = 'usageKeyFingerprintSecret'`)?.value;
  if (!secret) throw new Error("usage key fingerprint secret is unavailable");
  return `external:${createHmac("sha256", secret).update(apiKey).digest("hex").slice(0, 32)}`;
}
...
function importLegacyUsage(adapter, data) {
  ...
  for (const e of data.history || []) {
    ...
    adapter.run(
      `INSERT INTO usageHistory(timestamp, provider, model, connectionId, apiKey, apiKeyId, requestId, startedAt, completedAt, endpoint, promptTokens, completionTokens, cost, status, tokens, meta) VALUES(?, ?, ?, ?, NULL, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        timestamp,
        e.provider || null, e.model || null, e.connectionId || null, legacyUsageApiKeyId(adapter, e.apiKey), timestamp, timestamp, e.endpoint || null,
  ...
  // Legacy dailySummary embeds raw API keys. It is only a cache of history and
  // is intentionally not imported; exact history is rebuilt on read.
...
// ─── Main entry ──────────────────────────────────────────────────────────
export async function runMigrationOnce(adapter, { force = false } = {}) {
  if (_migratedAdapters.has(adapter) && !force) return;
