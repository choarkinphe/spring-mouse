          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1680px] border-collapse text-xs">
              ...
              <tbody className="divide-y divide-border/60">
                {loading ? (
                  <tr><td colSpan="12" className="px-5 py-14 text-center text-sm text-text-muted"><span className="material-symbols-outlined mr-2 animate-spin align-[-4px] text-[18px]">progress_activity</span>正在读取调用明细…</td></tr>
                ) : error ? (
                  <tr><td colSpan="12" className="px-5 py-14 text-center text-sm text-rose-600">{error}</td></tr>
                ) : !data.details?.length ? (
                  <tr><td colSpan="12" className="px-5 py-14 text-center text-sm text-text-muted">当前筛选范围内没有调用记录。</td></tr>
