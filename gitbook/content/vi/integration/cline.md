...
Tích hợp Spring Mouse với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Spring Mouse.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- Spring Mouse đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint Spring Mouse:

**Cho Spring Mouse cục bộ:**
```
http://localhost:8008/v1
```

**Cho Spring Mouse cloud:**
```
https://spring-mouse.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint Spring Mouse
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key Spring Mouse của bạn
2. Bạn có thể tìm API key trong Spring Mouse dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-spring-mouse-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình Spring Mouse
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng Spring Mouse để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua Spring Mouse
...
3. Nhận giải thích AI qua Spring Mouse

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Spring Mouse để hiểu context và thực hiện thay đổi
...
1. Xác minh Spring Mouse đang chạy: `curl http://localhost:8008/health`
2. Kiểm tra base URL đúng và bao gồm `/v1`
3. Đảm bảo không firewall nào chặn port 8008
...
2. Xác minh Spring Mouse instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Spring Mouse để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng Spring Mouse cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://spring-mouse.com`
2. Đảm bảo bạn đã cấu hình API key trong Spring Mouse cloud dashboard
...
## Tích hợp với Tính năng Spring Mouse

### Định tuyến Model

Spring Mouse tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Spring Mouse tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Spring Mouse dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
