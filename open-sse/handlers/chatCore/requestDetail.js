...
export function extractUsageFromResponse(responseBody) {
  ...
  if (usageMetadata) {
    ...
    let candidateTokens = Number(usageMetadata.candidatesTokenCount) || 0;

    // Gemini's visible candidates exclude thoughts, but its total includes
    // them. Keep completion cache/cost accounting aligned with that total.
...
export function saveUsageStats({ provider, model, tokens, connectionId, apiKey, endpoint, sourceIp, appName, userAgent, sourceUrl, requestId, startedAt, status = "success", label = "USAGE", silent = false }) {
  ...
  if (!silent && (inTokens > 0 || outTokens > 0)) {
