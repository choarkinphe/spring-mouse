...
import { findHeadroomBinary, findHeadroomBinaryViaPython, findPython310, HEADROOM_COMPRESSION_EXTRAS, EXTRA_MARKERS, getInstalledHeadroomExtras } from "./detect.js";
...
export async function startHeadroomProxy({ port = DEFAULT_PORT, codeAware = false, kompress = true } = {}) {
  ...
  const binary = findHeadroomBinary() || findHeadroomBinaryViaPython(findPython310());
  if (!binary) {
...
// Run one pip install attempt, appending output to the install log.
// Returns the child's exit code.
function runPipInstall(py, args, logFlag) {
  const outFd = fs.openSync(INSTALL_LOG_FILE, logFlag);
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code) => { fs.closeSync(outFd); resolve(code); });
  });
}

function readInstallLogTailForRetry() {
  try {
    return fs.readFileSync(INSTALL_LOG_FILE, "utf8");
  } catch { return ""; }
}
...
export async function installHeadroomExtras(extras = []) {
  ...
  // 不再要求预装 headroom-ai：spec 始终包含 [proxy] 基础包，
  // 从零安装与升级走同一条 pip 流程。
  ...
  let code = await runPipInstall(py, [...baseArgs, spec], "w");

  // PEP 668（uv/Homebrew 等托管 Python）回退：带 --break-system-packages 重试一次。
  if (code !== 0) {
    const logText = readInstallLogTailForRetry();
    if (logText.includes("externally-managed-environment")) {
      code = await runPipInstall(py, [...baseArgs, "--break-system-packages", spec], "a");
    }
  }

  if (code === 0) {
    const status = getInstalledHeadroomExtras(py);
    return { success: true, code, spec, extras: requested, ...status };
  }
