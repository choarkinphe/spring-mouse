...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("spring-mouse");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "spring-mouse-general-purpose",
      explore: "spring-mouse-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.spring-mouse\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.spring-mouse-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.spring-mouse-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# spring-mouse-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.spring-mouse-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("spring-mouse-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.spring-mouse-");
    expect(reset).not.toContain("spring-mouse-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("spring-mouse-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("spring-mouse-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("spring-mouse-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("spring-mouse-explore");
    expect(getGrokSubagentSlot("plan")).toBe("spring-mouse-plan");
