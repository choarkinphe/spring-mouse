...
Các vấn đề và giải pháp phổ biến khi dùng Spring Mouse.
...
1. **Auto-refresh (mặc định):**
   Spring Mouse tự refresh tokens. Đợi 30 giây rồi thử lại.
...
**Vấn đề:** Lỗi "ECONNREFUSED" hoặc "Cannot connect to localhost:8008".

**Nguyên nhân:**
- Spring Mouse không chạy
- Port 8008 bị chặn
- Firewall chặn kết nối

**Giải pháp:**

1. **Khởi động Spring Mouse:**
   ```bash
   spring-mouse
   ```
   Dashboard sẽ mở tại http://localhost:3000

2. **Xác minh port 8008:**
   ```bash
...
- Spring Mouse bị crash
- Vấn đề cache browser

**Giải pháp:**

1. **Kiểm tra Spring Mouse có chạy không:**
   ```bash
   # Check process
