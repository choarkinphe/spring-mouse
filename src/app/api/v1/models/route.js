...
// and break recursive loops between spring-mouse instances connected to each other.
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  for (const combo of combos) {
    ...
    const entry = {
      id: combo.name,
      object: "model",
      // cc-switch groups fetched models by `owned_by`. Keep ungrouped routes in
      // the established combo bucket while projecting configured groups directly.
      owned_by: groupName || "combo",
      is_combo: true,
    };
    if (combo.kind === "webSearch" || combo.kind === "webFetch") {
      entry.kind = combo.kind;
    }
    const capabilities = combo.capabilities || {};
    if (Number.isInteger(capabilities.contextWindow) && capabilities.contextWindow > 0) {
      entry.context_window = capabilities.contextWindow;
    }
    if (capabilities.vision === true || capabilities.audioInput === true) {
      entry.capabilities = {
        ...(capabilities.vision === true ? { vision: true } : {}),
        ...(capabilities.audioInput === true ? { audio_input: true } : {}),
      };
    }
    models.push(entry);
  }

  // Public /models endpoints expose routing entrypoints only. Direct provider
  // models remain available internally for dashboard model counts and management.
