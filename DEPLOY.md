...
- 该值只是软调度阈值，不是强制限流；所有账号达到阈值后仍会选择占用比例最低的账号，并继续记录溢出请求。降低该值不能实现单账号硬保护；
- 活跃请求使用独立租约（默认 90 秒、每 30 秒批量续租），正常结束、异常、取消时释放；`SPRING_MOUSE_CONNECTION_SLOT_TTL_SECONDS` 控制租约长度，不是请求超时时间；
- 路由和热缓存使用独立的 Redis 连接，单次操作默认最多等待 200ms（`SPRING_MOUSE_ROUTING_REDIS_TIMEOUT_MS`）；不可用时回退到数据库读取 / 进程内软调度，不改动持久用量队列的语义。

20 人多 Codex 窗口场景，应按每人 3–5 个同时工作的窗口验收，即 60–100 路活跃请求。隔离压测方法、修改前后数据和限制见 `docs/PERFORMANCE-MULTIWINDOW.md`。已登录管理员可读取 `/api/system/status` 的 `concurrency` 字段，查看本进程路由 P95、事件循环延迟、活跃租约和路由 Redis 超时数。
