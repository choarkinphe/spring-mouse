...
トラブルシューティングのヒント付きのSpring Mouse詳細インストールガイド。
...
どこからでも使用できるようにSpring Mouseをグローバルインストール:
...
**利点:**
- ✅ どのディレクトリからでも実行
- ✅ シンプルなコマンド: `spring-mouse`
- ✅ `npm update -g spring-mouse` で自動更新
...
**何が起こるか:**
1. サーバーが `http://localhost:8008` で起動
2. ダッシュボードが自動的にブラウザで開く
3. `~/.spring-mouse` にデータディレクトリが作成される
4. APIキーが自動生成される
