...
export function calculateCostFromTokens(tokens, pricing) {
  ...
  // Canonical usage defines completion_tokens as all generated output, including
  // reasoning/thinking tokens when the provider reports them separately. Charge the
  // visible portion at the output price and the reasoning subset once at its own price.
  const outputTokens = tokens.completion_tokens || tokens.output_tokens || 0;
  const reasoningTokens = Math.min(Math.max(tokens.reasoning_tokens || 0, 0), outputTokens);
