...
describe("cloudflared PID ownership", () => {
  ...
  beforeEach(() => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "spring-mouse-tunnel-pid-"));
  ...
  it("releases PID and process ownership for the exiting child only", () => {
    ...
    expect(source.match(/clearPid\(cloudflaredProcess\.pid\)/g)).toHaveLength(2);
    expect(source.match(/runtime\.child === cloudflaredProcess/g)).toHaveLength(2);
  });
});
