...
// Collect PIDs of all spring-mouse-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from the app install dir (current spring-mouse
        // or legacy spring-mouse layout) or wrapper cli.js
        const isAppProcess = lower.includes("spring-mouse") ||
          lower.includes("spring-mouse") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("spring-mouse") ||
          line.includes("spring-mouse") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "spring-mouse");
  }
  return path.join(os.homedir(), ".spring-mouse");
}
...
// Resolve npx/spring-mouse binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx spring-mouse` — works regardless of global bin path changes after npm i -g
