...
Spring Mouse is compatible with any tool that supports the OpenAI API format. This guide covers generic integration patterns for various tools and custom applications.

## Overview

Spring Mouse provides an OpenAI-compatible API endpoint that works with:
...
Any OpenAI-compatible tool can connect to Spring Mouse using these settings:
...
**Problem:** Cannot connect to Spring Mouse
```bash
...
**Solution:**
- Verify Spring Mouse is running
- Check port 8008 is not blocked
...
- Check available models: `curl http://localhost:8008/v1/models`
...
- Check network connection to Spring Mouse
