...
// useful for the Spring Mouse proxy use case, so we don't track them here.
...
// Fallback: derive the interpreter's script dir via sysconfig (uv/Homebrew
// managed Pythons install console scripts next to the interpreter, which is
// usually not on PATH). Returns the headroom binary path or null.
export function findHeadroomBinaryViaPython(python) {
  ...
  try {
    ...
    if (!out) return null;
    const candidate = IS_WIN ? path.join(out, "headroom.exe") : path.join(out, "headroom");
...
export async function getHeadroomStatus(url) {
  ...
  const path = findHeadroomBinary() || findHeadroomBinaryViaPython(python);
  const running = await probeProxyRunning(url);
  const localUrl = isLoopbackHeadroomUrl(url);
  // Sidecar/remote deployments (docker-compose headroom service): there is no
  // local CLI or python inside the app container by design — the proxy at
  // HEADROOM_URL is the source of truth. Treat a reachable proxy as
  // installed so the dashboard renders the compression panel.
  const installed = Boolean(path) || running;
  // Extras (pip packages) are only meaningful for the local install; in
  // sidecar mode the image ships its own capability set and pip is absent,
  // so report the extras as on without probing.
  const extrasStatus = path
    ? getInstalledHeadroomExtras(python)
    : running
      ? { installed: true, version: null, extras: { code: true, ml: true } }
  ...
  return {
    ...
    python,
    localUrl,
    canStart: Boolean(path) && localUrl,
