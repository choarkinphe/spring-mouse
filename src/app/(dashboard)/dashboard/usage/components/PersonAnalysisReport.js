...
function buildAnalysis(people, totals) {
...
export default function PersonAnalysisReport({ stats, timeRange }) {
  ...
  const people = useMemo(() => Object.entries(stats?.byUser || {}).map(([key, item]) => ({
    key,
    ...item,
    totalTokens: (item.promptTokens || 0) + (item.completionTokens || 0),
  })), [stats]);
  const totals = useMemo(() => ({ totalRequests: stats?.totalRequests || 0, totalTokens: (stats?.totalPromptTokens || 0) + (stats?.totalCompletionTokens || 0), totalCost: stats?.totalCost || 0 }), [stats]);
  const analysis = useMemo(() => buildAnalysis(people, totals), [people, totals]);
