<div align="center">
  <img src="../images/spring-mouse.png?1" alt="Bảng điều khiển Spring Mouse" width="800"/>
  
  # Spring Mouse - Free AI Router & Token Saver
  ...
  **Kết nối tất cả công cụ AI Code (Claude Code, Codex, Cursor, Cline, Copilot, Antigravity...) tới 40+ Nhà cung cấp AI & 100+ Mô hình.**
  ...
  [![License](https://github.com/decolua/spring-mouse/blob/main/LICENSE)](https://github.com/decolua/spring-mouse/blob/main/LICENSE)
  
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://spring-mouse.com)
</div>

---

## 🤔 Tại sao chọn Spring Mouse?
...
**Spring Mouse giải quyết vấn đề này:**
...
🎉 Bảng điều khiển (Dashboard) sẽ tự động mở tại `http://localhost:8008`
...
Gói kho lưu trữ này là riêng tư (`spring-mouse-app`), vì vậy việc chạy từ nguồn/Docker là cách phát triển cục bộ mặc định.
...
URL mặc định:
- Bảng điều khiển Dashboard: `http://localhost:8008/dashboard`
- API tương thích OpenAI: `http://localhost:8008/v1`
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - Spring Mouse + Claude Code MIỄN PHÍ
  
[![Thiết lập Spring Mouse + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập Spring Mouse
...
Spring Mouse hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → Spring Mouse dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân Spring Mouse **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của Spring Mouse

**Thực tế Thanh toán Spring Mouse:**

✅ **Phần mềm Spring Mouse = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **Spring Mouse không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, Spring Mouse chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị Spring Mouse tính phí không?</b></summary>

**Không.** Spring Mouse là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, Spring Mouse chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân Spring Mouse** → **Không bao giờ tính phí bất cứ thứ gì, ever**

Spring Mouse là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
Spring Mouse chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và Spring Mouse giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải Spring Mouse.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - Spring Mouse giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của Spring Mouse ngăn chặn các khoản phí bất ngờ:

**Kịch bản:** Bạn đang trong giai đoạn code nước rút và vượt qua các hạn mức

**Không có Spring Mouse:**
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và Spring Mouse sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. Spring Mouse theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển spring-mouse]
...
```json
{
  "agents": {
    "defaults": {
      "model": {
        "primary": "spring-mouse/if/glm-4.7"
      }
    }
  },
  "models": {
...
> **Lưu ý:** OpenClaw chỉ hoạt động với Spring Mouse cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
# Hoặc sử dụng PM2
npm install -g pm2
pm2 start --name spring-mouse -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name spring-mouse \
  -p 8008:8008 \
  --env-file /root/dev/spring-mouse/.env \
  -v spring-mouse-data:/app/data \
  -v spring-mouse-usage:/root/.spring-mouse \
  spring-mouse
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.spring-mouse/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.spring-mouse` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
| `PORT` | framework default | Cổng dịch vụ (`8008` trong các ví dụ) |
| `HOSTNAME` | framework default | Bind host (Docker mặc định là `0.0.0.0`) |
| `NODE_ENV` | runtime default | Đặt `production` để triển khai |
| `BASE_URL` | `http://localhost:8008` | URL cơ sở nội bộ phía máy chủ được sử dụng bởi các tác vụ đồng bộ đám mây |
| `CLOUD_URL` | `https://spring-mouse.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://spring-mouse.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
| `ENABLE_REQUEST_LOG_FILE_DUMPS` | `false` | Bật log request/response dưới `logs/` |
...
- Lịch sử sử dụng và log: `~/.spring-mouse/usage.json` và `~/.spring-mouse/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOG_FILE_DUMPS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.spring-mouse` và độc lập với `DATA_DIR`.
...
**Token OAuth hết hạn**
- Tự động làm mới bởi Spring Mouse
...
- Đặt `PORT=8008` và `NEXT_PUBLIC_BASE_URL=http://localhost:8008`

**Lỗi đồng bộ đám mây**
- Xác minh `BASE_URL` trỏ đến phiên bản đang chạy của bạn (ví dụ: `http://localhost:8008`)
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://spring-mouse.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://spring-mouse.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime Spring Mouse cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- Đặt `ENABLE_REQUEST_LOG_FILE_DUMPS=true`
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://spring-mouse.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- cục bộ (`http://127.0.0.1:8008/v1/chat/completions`): hoạt động với `stream=false` và `stream=true`.
...
- Endpoint đám mây công khai (`https://spring-mouse.com/v1/chat/completions`):
...
- **GitHub**: [github.com/decolua/9](https://github.com/decolua/spring-mouse)
...
Cảm ơn tất cả những người đã đóng góp giúp Spring Mouse tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=decolua/spring-mouse&max=100&columns=20&anon=1)](https://github.com/decolua/spring-mouse/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/decolua/spring-mouse.svg?variant=adaptive)](https://starchart.cc/decolua/spring-mouse)
...
Xem [Pull Requests](https://github.com/decolua/spring-mouse/pulls) để biết hướng dẫn chi tiết.
