...
export function filterToOpenAIFormat(body, opts = {}) {
  ...
  if (body.tools && Array.isArray(body.tools) && body.tools.length > 0) {
    body.tools = body.tools.map(tool => {
      // Some OpenAI clients omit or blank the parent type while keeping the
      // function wrapper. Strict Chat Completions providers (MiniMax M3) reject
      // that value as `invalid tool type`, so normalize it before passthrough.
      if (tool.function && (!tool.type || tool.type === OPENAI_BLOCK.FUNCTION)) {
        return { ...tool, type: OPENAI_BLOCK.FUNCTION };
      }
