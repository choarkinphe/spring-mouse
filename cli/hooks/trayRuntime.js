...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.spring-mouse/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/spring-mouse/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[spring-mouse][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[spring-mouse][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[spring-mouse][runtime] chmod tray bin failed: ${e.message}`);
  }
}
