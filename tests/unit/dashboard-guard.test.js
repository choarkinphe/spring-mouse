...
function localRequest(pathname, headers = {}) {
  return request(pathname, { "x-sm-peer-token": PEER_TOKEN, "x-sm-real-ip": "127.0.0.1", ...headers });
}
...
describe("dashboard guard public LLM API access", () => {
  ...
  it("allows loopback public LLM API without API key", async () => {
    const response = await proxy(localRequest("/v1/chat/completions", { host: "localhost:8008" }));

    expect(response).toBe(mocks.nextResponse);
    expect(mocks.validateApiKey).not.toHaveBeenCalled();
  });

  it("touches a valid API key on loopback requests instead of bypassing it", async () => {
    mocks.validateApiKey.mockResolvedValue(true);

    const response = await proxy(localRequest("/v1/models", {
      host: "localhost:8008",
      authorization: "Bearer sk-local-valid",
    }));

    expect(response).toBe(mocks.nextResponse);
    expect(mocks.validateApiKey).toHaveBeenCalledWith("sk-local-valid");
  });

  it("rejects remote CLI-token-only LLM requests without an API key", async () => {
  ...
  it("rejects a local request that supplies an invalid API key", async () => {
  ...
  it("exposes the Codex usage endpoint with its own API-key authentication", async () => {
    const response = await proxy(request("/api/codex/usage", { host: "router.example.com" }));
  ...
  it("allows loopback rewritten public LLM API without API key", async () => {
    const response = await proxy(localRequest("/api/v1/chat/completions", { host: "localhost:8008" }));
...
describe("dashboard IP access scope", () => {
  it("leaves API-key model endpoints outside the dashboard IP policy", () => {
    expect(__test__.shouldEnforceIpAccess("/dashboard")).toBe(true);
    expect(__test__.shouldEnforceIpAccess("/login")).toBe(true);
    expect(__test__.shouldEnforceIpAccess("/api/settings")).toBe(true);
    expect(__test__.shouldEnforceIpAccess("/v1/chat/completions")).toBe(false);
    expect(__test__.shouldEnforceIpAccess("/api/v1/chat/completions")).toBe(false);
    expect(__test__.shouldEnforceIpAccess("/codex/x")).toBe(false);
    expect(__test__.shouldEnforceIpAccess("/api/codex/usage")).toBe(false);
  });
});
