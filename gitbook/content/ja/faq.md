...
Spring Mouseに関する一般的な質問。

---

## Spring Mouseとは?

**Spring Mouseは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、Spring Mouse自体は100%無料でオープンソースです。**
...
**Spring Mouseはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## Spring Mouseをセルフホストできますか?

**はい! Spring Mouseは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.spring-mouse`)
...
**はい、Spring Mouseはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.spring-mouse`(またはカスタム`DATA_DIR`)にローカル保存
- Spring Mouseサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/decolua/spring-mouse/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.spring-mouse`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/decolua/spring-mouse/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [spring-mouse.com/docs](https://spring-mouse.com/docs)
