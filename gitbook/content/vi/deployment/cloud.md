...
Triển khai Spring Mouse trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
Tạo `/etc/nginx/sites-available/spring-mouse`:
...
# If NOT using reverse proxy, allow Spring Mouse ports
...
```bash
# Check if Spring Mouse is running
pm2 status
