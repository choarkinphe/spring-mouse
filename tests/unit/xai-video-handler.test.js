 ...
 *  - connection id surfaced via x-spring-mouse-connection-id
...
vi.mock("@/sse/services/ingressUsage.js", () => ({
  recordIngressUsage: vi.fn(async () => {}),
}));
...
beforeEach(() => {
  ...
  authMocks.authorizeApiKey.mockResolvedValue(null);
...
describe("handleVideoCreate", () => {
  ...
  it("returns the serving connection id in x-spring-mouse-connection-id", async () => {
    ...
    expect(res.headers.get("x-spring-mouse-connection-id")).toBe("conn-77");
  ...
  it("rotates to the next account on 401 (auth errors cannot have created a job)", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get("x-spring-mouse-connection-id")).toBe("conn-2");
