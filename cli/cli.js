...
// Subcommands (`, …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.spring-mouse/runtime
...
// Identifiers for killAllAppProcesses - only kill spring-mouse specifically
const PROCESS_IDENTIFIERS = [
  'spring-mouse'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "spring-mouse")
    : path.join(os.homedir(), ".spring-mouse");
}
...
// Kill all spring-mouse processes
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running spring-mouse/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "spring-mouse" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("spring-mouse") && (cmd.includes("cli.js") || cmd.includes("\\spring-mouse") || cmd.includes("/spring-mouse")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running spring-mouse/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "spring-mouse".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("spring-mouse") && (cmd.includes("cli.js") || cmd.includes("/spring-mouse")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Spring Mouse is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Spring Mouse is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "spring-mouse", "db.json") : path.join(".spring-mouse", "db.json"));
