...
Spring MouseをCline VSCode拡張機能と統合し、AIリクエストをSpring Mouseのインテリジェントルーティングシステム経由でルーティングします。
...
Base URLをSpring Mouseエンドポイントに設定:
...
**手順:**
1. **Base URL** フィールドにSpring Mouseエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
3. キーは `sk-spring-mouse-` で始まります

### 5. モデルを選択

1. **Model** ドロップダウンで、次のいずれかを実行:
   - 利用可能なモデルから選択 (Clineが自動検出した場合)
   - Spring Mouse設定からモデル名を手動で入力
...
2. チャット入力にメッセージを入力
3. Enterを押して送信
4. ClineはSpring Mouseを使用してリクエストを処理

### コード生成

1. Clineにコード生成を依頼: 「Create a React component for a login form」
2. ClineはSpring Mouseを使用してコードを生成
...
3. Spring Mouse経由でAIによる説明を取得

### ファイル操作

1. Clineにファイルの作成、変更、削除を依頼
2. ClineはSpring Mouseを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
2. Spring Mouseインスタンスが動作中で正常か確認
3. VSCodeウィンドウをリロードしてみる (Cmd/Ctrl + Shift + P → 「Reload Window」)
4. エラーについてSpring Mouseログを確認

## 高度な設定

### クラウドエンドポイントを使用

localhostの代わりにSpring Mouseクラウドエンドポイントを使用:

1. Cline設定で、Base URLを設定: `https://spring-mouse.com`
2. Spring MouseクラウドダッシュボードでAPIキーが設定されていることを確認
...
Spring Mouseは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、Spring Mouseは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

Spring Mouseダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
