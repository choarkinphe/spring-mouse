...
export function extractUsage(chunk) {
  ...
  // Most upstreams place usage at the chunk root, but some OpenAI-compatible
  // gateways attach it to the completed choice instead. Treat both shapes as
  // the same provider-authoritative usage record.
  const openAIUsage = chunk.usage || chunk.choices?.find((choice) => choice?.usage)?.usage;
  if (openAIUsage && typeof openAIUsage === "object" && openAIUsage.prompt_tokens !== undefined) {
    return normalizeUsage({
  ...
  if (usageMeta && typeof usageMeta === "object") {
    ...
    const reasoningTokens = Number(usageMeta.thoughtsTokenCount) || 0;
    const totalTokens = Number(usageMeta.totalTokenCount) || 0;
    let candidateTokens = Number(usageMeta.candidatesTokenCount) || 0;

    // Gemini reports thinking separately from visible candidate output, while
    // totalTokenCount includes both. Store completion as their sum so usage
    // totals and pricing include billed reasoning tokens. Some responses omit
    // candidatesTokenCount; in that case derive it from the provider total.
    if (candidateTokens === 0 && totalTokens > 0) {
      candidateTokens = Math.max(0, totalTokens - promptTokens - reasoningTokens);
    }
