...
export class DefaultExecutor extends BaseExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    if (transformed && typeof transformed === "object") {
      // Request final provider usage for configured OpenAI Chat Completions
      // transports. Without this, OpenAI streams omit exact usage and the
      // dashboard has to fall back to a character-count estimate.
      const transportFormat = credentials?.runtimeTransport?.format || this.config.format;
      if (stream && this.config.streamUsage === true && transportFormat === "openai") {
        transformed = {
          ...transformed,
          stream_options: { ...transformed.stream_options, include_usage: true },
        };
      }
