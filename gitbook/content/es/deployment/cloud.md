...
Despliega Spring Mouse en VPS o Docker para acceso remoto y uso en producción.
...
```bash
sudo mkdir -p /var/lib/spring-mouse
sudo chown $USER:$USER /var/lib/spring-mouse
```
...
```bash
# Ver logs
pm2 logs spring-mouse

# Reiniciar aplicación
pm2 restart spring-mouse

# Detener aplicación
pm2 stop spring-mouse
...
Crea `/etc/nginx/sites-available/spring-mouse`:
...
sudo ln -s /etc/nginx/sites-available/spring-mouse /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de Spring Mouse
...
tar -czf spring-mouse-backup-$(date +%Y%m%d).tar.gz /var/lib/spring-mouse

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/spring-mouse-$(date +\%Y\%m\%d).tar.gz /var/lib/spring-mouse
```
...
# Ver logs
pm2 logs spring-mouse --lines 100
...
# Conexiones de red
netstat -tulpn | grep -E '3000|8008'
```
...
pm2 env spring-mouse
```

### Nginx 502 Bad Gateway

```bash
# Verificar si Spring Mouse está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/spring-mouse
chmod 755 /var/lib/spring-mouse
```
