...
export async function getDeepseekUsage(apiKey = null, proxyOptions = null) {
  ...
  try {
    ...
    for (const b of balances) {
      ...
      // Credit pot: `total` remains for the quota progress UI, while `balance`
      // preserves the provider's actual money amount for compact dashboards.
      // Do not set absolute `remaining`: quota UI interprets that field as 0–100%.
